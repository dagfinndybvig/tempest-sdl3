...
// Simple text rendering for SDL2 (replaces SDL2_RenderText)
static void SDL2_RenderText(SDL_Renderer* renderer, float x, float y, const char* text) {
    // For now, just draw a simple rectangle to indicate text position
    // TODO: Implement proper text rendering for SDL2
    SDL_Rect rect = {(int)x, (int)y, (int)(strlen(text) * 8), 12};
...
static void DrawGameOverPrompt(AppContext* ctx, int w, int h) {
    ...
    SDL2_RenderText(ctx->renderer, tx, ty, touchPrompt);
#endif
}
...
static void DrawLandingPage(AppContext* ctx, int w, int h) {
    ...
    // Controls - Using SDL2_RenderText (built-in font)
    const char* controls[] = {
    ...
    for (int i = 0; i < 3; i++) {  // Changed from 4 to 3 since we removed one control
        ...
        SDL2_RenderText(ctx->renderer, tx, ty, controls[i]);
    }
    ...
    if (touchMessage) {
        ...
        SDL2_RenderText(ctx->renderer, tx, ty, touchMessage);
    }
#endif
...
void DrawBlaster(AppContext* ctx, int w, int h) {
    ...
    for(int i=0; i<3; i++) {
        SDL_RenderDrawLine(ctx->renderer, sx[i]-1, sy[i], sx[i+1]-1, sy[i+1]);
        SDL_RenderDrawLine(ctx->renderer, sx[i]+1, sy[i], sx[i+1]+1, sy[i+1]);
    }
    ...
    for(int i=0; i<3; i++) {
        SDL_RenderDrawLine(ctx->renderer, sx[i], sy[i], sx[i+1], sy[i+1]);
    }
...
void DrawDigit(SDL_Renderer* renderer, int digit, float x, float y, float size) {
    ...
    if (segments[digit][0]) SDL_RenderDrawLine(renderer, x, y, x + size, y);
    if (segments[digit][1]) SDL_RenderDrawLine(renderer, x, y, x, y + size);
    if (segments[digit][2]) SDL_RenderDrawLine(renderer, x + size, y, x + size, y + size);
    if (segments[digit][3]) SDL_RenderDrawLine(renderer, x, y + size, x + size, y + size);
    if (segments[digit][4]) SDL_RenderDrawLine(renderer, x, y + size, x, y + 2 * size);
    if (segments[digit][5]) SDL_RenderDrawLine(renderer, x + size, y + size, x + size, y + 2 * size);
    if (segments[digit][6]) SDL_RenderDrawLine(renderer, x, y + 2 * size, x + size, y + 2 * size);
}

// Empty audio callback for SDL2 version
void AudioCallback(void* userdata, SDL_AudioStream* stream, int len, int total_amount) {
    // Audio disabled in SDL2 version
}
...
void DrawHUD(AppContext* ctx, int w, int h) {
    ...
    for (int i = 0; i < ctx->lives; i++) {
        ...
        float y = 20.0f;
        float size = 10.0f;
        SDL_RenderDrawLine(ctx->renderer, x, y, x, y + size);
        SDL_RenderDrawLine(ctx->renderer, x, y + size, x + size, y + size);
        SDL_RenderDrawLine(ctx->renderer, x + size, y + size, x + size, y);
    }
    ...
    if (!ctx->superzapperUsed) {
        ...
        SDL_RenderDrawLine(ctx->renderer, 20, 45, 30, 45);
        SDL_RenderDrawLine(ctx->renderer, 25, 40, 25, 50); // Small "+"
    }
    ...
    if (ctx->showTouchControls) {
        ...
        // Draw labels
        SDL_SetRenderDrawColor(ctx->renderer, 255, 255, 255, 255);
        SDL2_RenderText(ctx->renderer, centerX - 50.0f, centerY - 10.0f, "SWIPE TO ROTATE");
        SDL2_RenderText(ctx->renderer, centerX - 30.0f, centerY + 10.0f, "TAP TO FIRE");
        SDL2_RenderText(ctx->renderer, w * 0.85f - 20.0f, h * 0.85f, "ZAP");
        
        // Draw swipe direction arrows
        SDL2_RenderText(ctx->renderer, centerX - radius - 40.0f, centerY - 20.0f, "CCW");
        SDL2_RenderText(ctx->renderer, centerX + radius + 10.0f, centerY - 20.0f, "CW");
    }
#endif
}
...
static void DrawHighScoreDisplayScreen(AppContext* ctx, int w, int h) {
    ...
    SDL2_RenderText(ctx->renderer, titleX, tableY + 12.0f, "HIGH SCORE TABLE");
    ...
    for (int i = 0; i < MAX_HIGHSCORES; i++) {
        ...
        SDL2_RenderText(ctx->renderer, tableX + 20.0f, yPos, scoreText); // Moved 10 pixels right
    }
    ...
    if (isEditing) {
        ...
        if ((SDL_GetTicks() / 500) % 2 == 0) {
            SDL_RenderDrawLine(ctx->renderer, cursorX, yPos, cursorX, yPos + 4.0f); // Scaled cursor height
        }
    ...
    if (!isEditing) {
        ...
        SDL2_RenderText(ctx->renderer, instrX, tableY + 150.0f, "PRESS R TO CONTINUE");
        
        // Add tap instruction for web version
#ifdef __EMSCRIPTEN__
        float tapInstrX = tableX + (200.0f - (float)strlen("OR TAP TO CONTINUE") * 8.0f) / 2.0f;
        SDL2_RenderText(ctx->renderer, tapInstrX, tableY + 160.0f, "OR TAP TO CONTINUE");
#endif
    } else {
        ...
        SDL2_RenderText(ctx->renderer, (w - strlen(instr) * 8) * 0.5f, h * 0.55f, instr);
        ...
        SDL2_RenderText(ctx->renderer, confirmTextX, confirmTextY, confirmText);
#else
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_FINGERDOWN || event.type == SDL_FINGERMOTION) {
            ...
            if (ctx->state == STATE_PLAYING) {
                ...
                else if (event.type == SDL_FINGERMOTION && !ctx->isSwiping) {
        ...
        if (event.type == SDL_KEYDOWN) {
            ...
            if (ctx->state == STATE_GAMEOVER) {
                ...
                if (RestartWithShape(ctx, event.key.keysym.scancode, event.key.keysym.sym, event.key.repeat)) {
                    ctx->state = STATE_PLAYING;
                    continue;
                }
            ...
            if (event.key.keysym.scancode == SDL_SCANCODE_LEFT) ctx->playerSegment = (ctx->playerSegment + 1) % NUM_SIDES;
            if (event.key.keysym.scancode == SDL_SCANCODE_RIGHT) ctx->playerSegment = (ctx->playerSegment - 1 + NUM_SIDES) % NUM_SIDES;
            if (ctx->state == STATE_PLAYING) {
                if (event.key.keysym.scancode == SDL_SCANCODE_0 || event.key.keysym.sym == SDLK_0) {
                    SelectTunnelShape(ctx, TUNNEL_IRREGULAR);
                }
                if (event.key.keysym.scancode == SDL_SCANCODE_1 || event.key.keysym.sym == SDLK_1) {
                    SelectTunnelShape(ctx, TUNNEL_CIRCLE);
                }
                if (event.key.keysym.scancode == SDL_SCANCODE_2 || event.key.keysym.sym == SDLK_2) {
                    SelectTunnelShape(ctx, TUNNEL_SQUARE);
                }
                if (event.key.keysym.scancode == SDL_SCANCODE_3 || event.key.keysym.sym == SDLK_3) {
            ...
            if (event.key.keysym.scancode == SDL_SCANCODE_Z && !ctx->superzapperUsed) {
            ...
            if (ctx->state == STATE_HIGHSCORE_DISPLAY) {
                ...
                if (isEditing) {
                    ...
                    } else if (event.key.keysym.sym >= 'a' && event.key.keysym.sym <= 'z' && !event.key.repeat) {
                        // Letter keys (ignore repeats to prevent multiple registrations)
                        if (ctx->nameEntryCursorPos < 19) {
                            ctx->newHighScoreName[ctx->nameEntryCursorPos] = (char)toupper(event.key.keysym.sym);
                            ctx->nameEntryCursorPos++;
            ...
            if (ctx->showHighScores && event.key.keysym.scancode == SDL_SCANCODE_R) {
                ctx->showHighScores = false;
                ResetGame(ctx);
                ctx->state = STATE_PLAYING;
            }
    ...
    if (ctx->state == STATE_LANDING) {
        ...
        for (int r = 0; r < NUM_RINGS; r++) {
            ...
            for (int s = 0; s < NUM_SIDES; s++) {
                ...
                SDL_RenderDrawLine(ctx->renderer, x1, y1, x3, y3);
        ...
        for (int i = 0; i < MAX_ENEMIES; i++) {
            if (ctx->enemies[i].active) {
                ...
                SDL_RenderDrawLine(ctx->renderer, sx[0], sy[0], sx[1], sy[1]);
                SDL_RenderDrawLine(ctx->renderer, sx[2], sy[2], sx[3], sy[3]);
                }
                }
                ...
                SDL_SetRenderDrawColor(ctx->renderer, 255, 255, 255, 255);
                for(int k=0; k<4; k++) {
                    SDL_RenderDrawLine(ctx->renderer, sx[k], sy[k], sx[(k+1)%4], sy[(k+1)%4]);
                }
                }
                }
        // Draw Shots
        ...
        for (int i = 0; i < MAX_SHOTS; i++) {
            if (ctx->shots[i].active) {
                ...
                SDL_RenderDrawLine(ctx->renderer, x1 - 1.0f, y1, x3 - 1.0f, y3);
                SDL_RenderDrawLine(ctx->renderer, x1 + 1.0f, y1, x3 + 1.0f, y3);
                SDL_RenderDrawLine(ctx->renderer, x1, y1 - 1.0f, x3, y3 - 1.0f);
                SDL_RenderDrawLine(ctx->renderer, x1, y1 + 1.0f, x3, y3 + 1.0f);
                SDL_RenderDrawLine(ctx->renderer, x2 - 1.0f, y2, x4 - 1.0f, y4);
                SDL_RenderDrawLine(ctx->renderer, x2 + 1.0f, y2, x4 + 1.0f, y4);
                SDL_RenderDrawLine(ctx->renderer, x2, y2 - 1.0f, x4, y4 - 1.0f);
                SDL_RenderDrawLine(ctx->renderer, x2, y2 + 1.0f, x4, y4 + 1.0f);

                SDL_SetRenderDrawColor(ctx->renderer, 255, 0, 0, 255);
                SDL_RenderDrawLine(ctx->renderer, x1, y1, x3, y3);
                SDL_RenderDrawLine(ctx->renderer, x2, y2, x4, y4);
            }
        }
        SDL_SetRenderDrawBlendMode(ctx->renderer, SDL_BLENDMODE_BLEND);
...
int main(int argc, char* argv[]) {
    ...
    // SDL2 audio initialization - simplified for now
    SDL_AudioSpec targetSpec;
    targetSpec.freq = 44100;
    targetSpec.format = AUDIO_F32;
    targetSpec.channels = 2;
    targetSpec.samples = 1024;
    targetSpec.callback = NULL; // We'll handle audio differently for SDL2
    targetSpec.userdata = NULL;
    
    // For now, disable audio to get SDL2 version working
    // TODO: Implement proper SDL2 audio system
    ctx.audio.stream = NULL;
    printf("Audio disabled in SDL2 version (TODO: implement SDL2 audio)\n");
